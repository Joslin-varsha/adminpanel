...
export default function SkillsPage() {
  ...
  const skillsList = [
    { id: 1, skillId: 'SK-001', name: 'Deep Cleaning', description: 'Extra time & effort required', status: 'Active' },
    { id: 2, skillId: 'SK-002', name: 'Pipe Installation', description: 'Advanced plumbing work', status: 'Active' },
    { id: 3, skillId: 'SK-003', name: 'AC Repair', description: 'Skilled technician required', status: 'Inactive' },
    { id: 4, skillId: 'SK-004', name: 'Window Cleaning', description: 'High-rise window service', status: 'Active' },
    { id: 5, skillId: 'SK-005', name: 'Electrical Wiring', description: 'Complex wiring installations', status: 'Active' },
    { id: 6, skillId: 'SK-006', name: 'Drain Unclogging', description: 'Emergency drain service', status: 'Active' },
    { id: 7, skillId: 'SK-007', name: 'Carpet Cleaning', description: 'Specialized equipment needed', status: 'Active' },
    { id: 8, skillId: 'SK-008', name: 'Water Heater Repair', description: 'Gas & electric heater service', status: 'Active' },
    { id: 9, skillId: 'SK-009', name: 'Light Fixture Install', description: 'Chandelier & fixture mounting', status: 'Inactive' },
    { id: 10, skillId: 'SK-010', name: 'Sanitization Service', description: 'Deep disinfection treatment', status: 'Active' }
  ];
    ...
    <div className="flex h-screen overflow-hidden bg-white dark:bg-slate-950 font-sans">
             ...
             <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto">
               ...
               <div className="relative w-full sm:w-48">
                 <select 
                   ...
                   className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-slate-900 dark:text-slate-100 transition-shadow shadow-sm appearance-none cursor-pointer"
                 >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm mb-6">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-sm">
                  {currentSkills.map((skill) => (
                    <tr 
                      key={skill.id} 
                      className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${openDropdownId === skill.id ? 'relative z-50' : ''}`}
                    >
                      <td className="px-4 py-4 text-slate-400 text-center">{skill.id}</td>
                      <td className="px-4 py-4 text-slate-500 dark:text-slate-400">{skill.skillId}</td>
                      <td className="px-4 py-4 text-slate-600 dark:text-slate-300 font-medium">{skill.name}</td>


                      <td className="px-4 py-4 text-slate-500 dark:text-slate-400 max-w-xs truncate">{skill.description}</td>
                       <td className="px-4 py-4">
                        <div className="flex items-center gap-1.5">
                          <div className={`w-1.5 h-1.5 rounded-full ${skill.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
                          <span className={`text-[10px] uppercase tracking-wider ${getStatusBadgeStyle(skill.status)}`}>
                            {skill.status}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-4 pr-8 text-right relative">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenDropdownId(openDropdownId === skill.id ? null : skill.id);
                          }}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30">
              <div className="text-sm text-slate-500 dark:text-slate-400">
                Showing <span className="font-medium text-slate-900 dark:text-white">{filteredSkills.length === 0 ? 0 : indexOfFirstItem + 1}</span> to <span className="font-medium text-slate-900 dark:text-white">{Math.min(indexOfLastItem, filteredSkills.length)}</span> of <span className="font-medium text-slate-900 dark:text-white">{filteredSkills.length}</span> entries
              </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">

            {/* Modal Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              <h2 className="text-xl font-bold text-slate-800 dark:text-white font-serif">Add New Skill</h2>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-8">
              <form className="space-y-6">
                ...
                <div>
                  ...
                  <textarea placeholder="Type here" rows="3" className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white text-sm text-slate-900 dark:text-slate-100 transition-colors resize-none"></textarea>
                </div>
              </form>
            </div>
            ...
            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                <button
                  className="flex-1 sm:flex-none px-6 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-sm"
                  onClick={() => setIsAddModalOpen(false)}
                >
                  Save Skill
                </button>
